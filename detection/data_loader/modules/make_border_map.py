...
class MakeBorderMap():
    ...
    def __call__(self, data: dict) -> dict:
        ...
        ignore_mask = np.ones(im.shape[:2], dtype=np.float32)
        
        for i in range(len(text_polys)):

            if ignore_tags[i]:
                self.draw_ignore_mask(text_polys[i], ignore_mask)
            else:
    ...
    def draw_ignore_mask(self, polygon, ignore_mask):
        ...
        cv2.fillPoly(ignore_mask, [padded_polygon.astype(np.int32)], 0.0)
